    ...
    createChildSpan(options) {
        ...
        if (rootSpan.type === constants_1.SpanType.ROOT) {
            ...
            if (rootSpan.trace.spans.length >= this.config.spansPerTraceHardLimit) {
                ...
                this.logger.warn(`TraceApi#createChildSpan: [${this.pluginNameToLog}] Creating phantom child span [${options.name}] because the trace with root span [${rootSpan.span.name}] has reached a limit of ${this.config.spansPerTraceHardLimit} spans. This is likely a memory leak.`);
                this.logger.warn([
            ...
            if (rootSpan.trace.spans.length === this.config.spansPerTraceSoftLimit) {
                ...
                this.logger.warn(`TraceApi#createChildSpan: [${this.pluginNameToLog}] Adding child span [${options.name}] will cause the trace with root span [${rootSpan.span.name}] to contain more than ${this.config.spansPerTraceSoftLimit} spans. This is likely a memory leak.`);
                this.logger.warn([
                    'TraceApi#createChildSpan: Please see',
                    'https://github.com/googleapis/cloud-trace-nodejs/wiki',
                    'for details and suggested actions.',
